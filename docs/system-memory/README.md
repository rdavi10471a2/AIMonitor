...
MCP edit sessions must declare the planned file set when the session is created, even for one-file edits. Each planned row carries the watched source path, relative file path, file name, owning project path, and project file name so later validation and indexing do not re-derive workflow intent from filesystem guesses. Agents must include known dependent callers, consumers, generated companions, and project/config inputs in that plan before review; validation is a guardrail for missed impact, not the primary dependency discovery mechanism. Planned MCP edits defer overlay compilation until all planned Working candidates exist, require all planned files to be staged before WinMerge review launch, perform WinMerge accept/reject decisions before the expensive post-accept index pass, run a full staged overlay build before the terminal planned decision completes for any accepted planned set, and use the planned file/project list for the post-accept refresh after the last planned file reaches a terminal decision.
