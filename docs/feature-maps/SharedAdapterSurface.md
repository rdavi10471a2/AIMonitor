...
- `AIMonitor.Data` owns SQLite schema and durable solution index state; there is no separate `AIMonitor.Storage` boundary.
...
- CLI/MCP adapters send log events to the pipe and fall back to direct file writes only when the hub is not running.
...
For adapter verification, callers invoke the CLI or MCP executable. WinForms receives adapter log events over a named pipe, writes the shared runtime log, and displays the events as they arrive. The log file is durable output, not the primary communication channel.
...
- CLI and MCP commands must share `.Data` services before adding adapter-specific payload shaping.
...
- `tests/unit/AIMonitor.Data.Tests/SolutionIndexStoreTests.cs` verifies document content hashes are persisted and returned.
...
- Failed pre-merge validation should be treated as a human gate, not a warning. On an interactive Windows desktop the CLI displays an OK/Cancel override dialog before launching WinMerge. If no dialog is available, Codex/Claude must ask the user in chat and use `--force-validation` only after explicit approval.
