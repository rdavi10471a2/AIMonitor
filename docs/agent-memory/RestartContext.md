...
- 2026-06-07 immediate restart pointer for MCP retest: current branch is `codex/planning-outer-loop`, pushed to PR #11 at commit `f2c6090` with the session-plan DTO spike. Read `docs/feature-maps/PlanningOuterLoopRedesign.md` before continuing. The new front-door contract is `start_monitor_session` followed by `set_monitor_session_plan(sessionId, taskId, iterationId, filesPlanned)` before watched-source edits; `record_diff_decision` now returns `sessionProgress` so the back door can tell whether a decided file is 1 of N or the final planned file. Focused tests passed after this change: `Mcp_session_multi_file_accept_flow_tracks_both_staged_records` and `Mcp_server_lists_monitor_tools_and_serves_index_queries`. After chat/plugin reset, first verify live MCP with `get_monitor_status`, then call `test_elicitation`, then use `get_current_task_context`. The intended live smoke is the standard harmless SchemaMCPRepository variable/property edit, but this time through MCP with `set_monitor_session_plan` before `refresh_file`, and checking `record_diff_decision.sessionProgress` after the accepted decision.
