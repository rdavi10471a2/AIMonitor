...
For new watched-project files, use `edit new --file <future-watched-path>`. This creates an empty monitor-owned Working candidate and later stages against a blank runtime review baseline. `edit launch-diff` opens WinMerge against monitor-owned runtime files for new-file review. The watched source file is created only after an explicit accepted decision verifies both the reviewed runtime content and the staged hash. Rejected new-file decisions leave the watched source absent.
