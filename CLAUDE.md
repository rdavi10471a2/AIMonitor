...
Codex parity starts with the CLI workflow edit loop. For watched-project edits, Codex should use `edit refresh` to create a monitor-owned working candidate, edit that candidate, then use `edit stage`, `edit launch-diff`, and `edit record-decision`. `edit replace-text` is primarily a Codex-safe local command path for exact replacements; Claude may keep using its MCP/editor surface when that surface already preserves stable local edits. `edit launch-diff` runs the full pre-merge validation gate before WinMerge. If validation fails, the user must explicitly approve the validation override dialog before WinMerge opens. If no interactive dialog is available, the agent must ask the user in chat and rerun with `--force-validation` only after explicit approval. Do not describe direct watched-source patching or silent candidate copying as the clean path.

For new watched-project files, use `edit new --file <future-watched-path>`. This creates an empty monitor-owned Working candidate and later stages against a blank review baseline. The watched file is created only by the WinMerge/operator save path and verified by `edit record-decision`.

After `edit record-decision` returns an accepted outcome, the next operation on that watched file must be `edit refresh`. This captures the watched-source bytes that WinMerge or the editor actually saved, including hashes and line endings.
